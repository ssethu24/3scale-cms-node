...
module.exports = class API3Scale {
  ...
  updateFile(id, file) {
    ...
    console.log(`${this.baseUrl}/${this.kindKey['file']}/${file.id}.json?provider_key=${this.providerKey}`);
    return fetch(`${this.baseUrl}/${this.kindKey['file']}/${id}.json?provider_key=${this.providerKey}`, {
      method: 'PUT',
      headers: form.getHeaders(),
      body: form,
  ...
  createSection(section) {
    console.log("cerate", section);
    let newSection = {};
    section.provider_key = this.providerKey;
    const title = section.title;
    section.title = section.system_name;
    return fetch(`${this.baseUrl}/${this.kindKey['section']}.json`, {
      ...
      .then((result) => {
        console.log(result);
        newSection = result.section;
        newSection.title = title;
        return this.updateSection(newSection.id, { title })
      })
      .then(() => newSection);
  }
